...
function splitValue(originalValue: any, cursorPosition: any, trigger: any) {
...
function getCharHeight(...elements: HTMLSpanElement[]) {
...
function getCaretPosition(element, trigger: any) {
    if (data(element, 'isInput')) {
        ...
        if (element.tagName === 'INPUT') {
            ...
            charHeight = isNaN(getCharHeight(clone, positioner)) ? 0 : getCharHeight(clone, positioner);
        }
        ...
        const caretPosition = getGlobalOffset(positioner);
        const charHeight = isNaN(getCharHeight(positioner)) ? 0 : getCharHeight(positioner);
...
const getNextNode = (node: { nextSibling: any; parentNode: any; }, root?: undefined) => {
    let nextNode;
    if (node.nextSibling)
...
const removeNodesBetween = (startContainer: HTMLElement, endContainer: HTMLElement) => {
    if (startContainer === endContainer) return;
    let node: any = getNextNode(startContainer);
...
const insertHtmlAfter = (node: { nextSibling: any; parentNode: { insertBefore: (arg0: ChildNode, arg1: any) => void; }; }, html: string) => {
...
const setValue = ({ element, trigger, suggestion, onChange }) => {
    if (data(element, 'isInput')) {
        ...
        if (suggestion.insertHtml) {
            ...
            function setSelection(focus: number, nodes: any[], method: string) {
                ...
                if (lastFocus === 0) {
                    ...
                    // range[method + 'After'](startContainer.parentNode);
                } else {
...
export default class AutoSuggest {
    ...
    onBlurHandler: () => void;
    onKeyDownHandler: (e: {
      preventDefault: () => void; keyCode: number;
    }) => void;
    onKeyUpHandler: (e: any) => void;
    value: any;
    constructor(options: any, ...inputs: any) {
        if (!options) {
        ...
        events: {
            const self = this;
            let activeSuggestionList: { trigger: any; } | null = null;
            ...
            this.onKeyDownHandler = function(e: { preventDefault: () => void; keyCode: number; }) {
            ...
            let keyUpIndex = 0;
            this.onKeyUpHandler = function(e: any) {
                if (handledInKeyDown) {
                    ...
                    self.suggestionLists.forEach((suggestionList) => {
                        if (suggestionList.regex.test(value)) {
                            ...
                            ((i, asyncReference) => {
                                ...
                                suggestionList.getSuggestions.call(this, match, (results: any) => {
